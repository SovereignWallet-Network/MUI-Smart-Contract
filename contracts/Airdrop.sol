 ...
 * is mined he may make `claim()` transaction again to claim incentivized tokens again. 
...
contract Airdrop is Withdrawable, Depositable, Destructible {
    ...
    ERC20 public token;
    bytes32 public rootHashIncentives;
    mapping (uint256 => mapping (uint256 => uint256)) vRedeemed;
    uint256 version = 0;
    ...
    /**
     * @dev Sets the hash root of incentives 
     * @param _rootHashIncentives bytes32
     */
    ...
    function isClaimed(uint256 index) public view returns (bool) {
        mapping (uint256 => uint256) redeemed = vRedeemed[version];
    ...
    function claim(uint256 index, uint256 amount, bytes32[] merkleProof) external {
        ...
        require(checkMerkleProof(index, msg.sender, amount, merkleProof));
